...
Decoder.prototype._transform = function _transform(input, encoding, done) {
  ...
  while (offset < input.length) {
    // i've seen something identifying itself as uTorrent/2210 creating torrents
    // that are padded out to 32768 bytes with null bytes... i wonder how common
    // this is? weird.
    if (this.states.length === 0 && input[offset] === 0) {
    ...
    if (this.states.length === 0) {
      return this.emit("error", Error("extra data was found after the torrent content"));
    }
    ...
    return this.emit("error", Error("got data that couldn't be parsed at offset " + (this.parsed + offset)));
  }
